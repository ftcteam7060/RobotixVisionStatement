...
public class CameraTestActivity extends Activity implements CvCameraViewListener2 {
    ...
    private static final ColorHSV lowerBoundRed = new ColorHSV((int) (275 / 360.0 * 255.0), (int) (0.100 * 255.0), (int) (0.100 * 255.0));
...
//        android.hardware.Camera.Parameters pam = cam.getCamera().getParameters();
        focalLength = cam.getCamera().getParameters().getFocalLength();
        cam.release();
...
//                    new Point(0, 8), 1.0f, new ColorGRAY(255), Drawing.Anchor.BOTTOMLEFT);
            ...
            Drawing.drawText(mRgba, Double.toString(beacon.getBeaconCenter().x) + ", " + Double.toString(beacon.getBeaconCenter().y),
                    new Point(0, 8), 1.0f, new ColorGRAY(255), Drawing.Anchor.BOTTOMLEFT);
        } catch (Exception e) {
